...
module.exports = class ColorinfoCommand extends Command {
    ...
    async run({ message, args, guild, channel, author, prefix, lang }) {
        ...
        let buffer = Buffer.from(svgcode
            ...
            .replace(/%colorsatur1%/gi, chroma.scale([color, chroma(color).set('hsv.s',1).alpha(chroma(color).get('rgba.a')).hex()])(1).hex())
            .replace(/%colorsatur2%/gi, chroma.scale([color, chroma(color).set('hsv.s',1).alpha(chroma(color).get('rgba.a')).hex()])(.75).hex())
            .replace(/%colorsatur3%/gi, chroma.scale([color, chroma(color).set('hsv.s',1).alpha(chroma(color).get('rgba.a')).hex()])(.5).hex())
            .replace(/%colorsatur4%/gi, chroma.scale([color, chroma(color).set('hsv.s',1).alpha(chroma(color).get('rgba.a')).hex()])(.25).hex())
