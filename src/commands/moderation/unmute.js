...
module.exports = class UnmuteCommand extends Command {
    ...
    async run({ author, args, message, guild }) {
        ...
        let muterole = await this.client.guildConfig.get(`${guild.id}.muteRole`).catch(() => null)
        ...
        if (typeof ch === 'string') {
            ...
            logchannel.send({ embeds: [this.client.embedder.modLog(guild, author, user, reason === '' ? 'Sem Motivo' : reason, 'UNMUTE')] })
        }
    }
};
