...
module.exports = class BanCommand extends Command {
    ...
    async run({ args, message, guild, author, channel, lang }) {
        ...
        if (typeof ch === 'string') {
            ...
            logchannel.send({ embeds: [this.client.embedder.modLog(guild, author, user, reason === '' ? this.client.locale(lang, 'NO_REASON') : reason, 'BAN')] }).catch(() => channel.send(this.client.locale(lang, 'ERROR_CANNOT_LOG')))
        }
    }
};
