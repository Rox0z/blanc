...
module.exports = class MuteCommand extends Command {
    ...
    async run({ args, message, guild, author, lang, channel, prefix }) {
        ...
        if (typeof ch === 'string') {
            ...
            logchannel.send({ embeds: [this.client.embedder.modLog(guild, author, user, reason === '' ? this.client.locale(lang, 'NO_REASON') : reason, 'MUTE', lang, Math.round((Date.now() + ms(args[1])) / 1000))] }).catch(() => channel.send(this.client.locale(lang, 'ERROR_CANNOT_LOG')))
        }
    }
};
